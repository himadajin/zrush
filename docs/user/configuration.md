...
ただし、1 回の一覧作りが読み込む履歴の総バイト数には固定の上限(256 KiB)があり、
`limit` 件に届く前にそこへ達した場合は、新しい方から入るところまでが対象になる。
`limit` を上げても待ち時間が際限なく延びることはなく、代わりに古い側が対象から外れる。
1 行が極端に長い履歴が多いと、`limit` が小さくてもこの上限に先に届く。
通常の履歴なら既定の 5000 は上限に届かず、そのまま全件が対象になる。

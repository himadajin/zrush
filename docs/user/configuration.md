...
```toml
[display]
max-lines = 10        # 一覧の最大行数(通常補完の位置表示を含む。1 行では候補のみ)
...
絞り込みは常駐ワーカーが持っている履歴に対して行うので、大きくするほど押してから出るまでの時間が延びる。

ワーカーが履歴をまとめて受け取るのは、そのワーカーで最初に履歴メニューを開いたときと、
他のシェルとの履歴共有などで食い違いを見つけたときだけで、以降のコマンドは 1 件ずつ渡す。
ワーカーが再起動した場合や zrush を re-source した場合は、次の履歴メニューでまとめて渡し直す。
まとめて渡すときの総バイト数には固定の上限(256 KiB)がある。

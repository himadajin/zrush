...
- 検証は入力通知を作る時点で行う。
  ...
  **フィンガープリント**(「Namespace Snapshot Synchronization」節の名前集合と
  `$PATH` 文字列からなる signature + PATH 各ディレクトリの mtime。
  `autocd` 有効または PATH に相対要素がある場合のみ `$PWD` を含める)の一致、
  **TTL 300 秒**(固定値)以内、latch が有効であること。
  signature は名前の個数ではなく名前の一覧そのものであり、
  個数が同じまま名前だけが置き換わった変化も不一致として捕まる。
  関数の一覧からは zrush 自身の名前空間(`_zrush` 接頭辞)を除く。
  zrush が定義・生成・削除する関数は候補集合の変化ではない。
  signature の算出は namespace snapshot と共有し、プロンプトごとの走査を 1 回にする。
  共有するのは算出だけで、保持と無効化はそれぞれの機構が独立に持つ
  (worker への供給状態はキャッシュの検証に影響せず、逆も同じ)。
...
- レイアウト(グループ分割・補完一覧の列優先グリッド・履歴一覧の単一列・見出しの省略・セル幅・ハイライト範囲・
  ...
  行数・列数・装飾範囲を独自に計算しない。
  行数予算(`rows`)は zsh が `max(1, min(max-lines, $LINES - 1))` から、
  桁数予算(`width`)は `$COLUMNS - 1` から都度算出して渡す(cli-protocol.md)。
  zsh は収集を制限しない(compsys が返す候補をすべて捕獲する)。
  補完一覧は隠し候補除外・マッチ判定・approximate 抑止後の全一致候補を対象とする。
  ランキング後の初出順でグループを並べ、グループ内はランキング順とする。
  この候補順を固定した表示窓を持ち、列幅は各グループの全一致候補から決める。
  同じ候補集合・クエリ・端末幅では、窓の移動で順序・列幅を変えない。
  窓がグループ途中から始まっても元の見出しを使う。見出し省略・空列圧縮は
  cli-protocol.md「Display Row Contents」の規則に従う。
  位置表示なしで全行予算を使っても全候補が一度に収まらない場合だけ、
  一覧最下部の専用行に選択位置 / 総一致候補数を `n/N` で表示する。見出しは数えず、
  未選択時は専用行を空欄にし、選択開始・解除で候補の配置を変えない。
  選択中は末尾までスクロールしても表示する。
  全候補が収まる場合は窓を先頭に戻し、位置表示を省略する。0 件なら一覧全体を消す。
  位置表示も行予算に含めるが、予算 1 行では候補を優先して位置表示を省略する。
...
- 端末リサイズ時: 描画プランは通知・要求の時点の `rows` / `width` に基づくスナップショットであり、
  ...
  通常補完の次の再描画では最新サイズを使い、操作対象の候補を窓内に保つ。
...
- 補完一覧の全体先頭候補での select-prev は選択を解除して通常状態に戻る(一覧テキストは残る)。
...
- 選択移動・列ジャンプの実現は、直前に適用したプランのナビゲーション表
  ...
  補完一覧も窓外への移動だけ worker に非同期で再描画を要求する。
  select-next / select-prev は画面上の位置によらず全体候補順の次 / 前の 1 件へ進む。
  全体末尾での select-next は停止する。窓内なら窓を動かさず、窓外なら対象が収まる
  最小量だけ先頭を動かす。サイズが変わらなければ同じグループ内では 1 候補分、グループ境界では見出しと候補を
  収めるために複数候補分を許す。ページ送りは行わない。左右は表示中の同じグループ内に留まる。
  補完一覧では select-next が `next`、select-prev が `prev` に対応する
  (履歴一覧での写像は「History Menu」節)。
- 通常補完の再描画は worker が保持する候補を使い、再収集・同期待機をしない。
  移動キーを取りこぼさず押下順に処理し、Enter と選択中の Tab は先行移動が終わった候補を挿入する。
  ブラウズ中は入力を変更しない。待機中の編集・カーソル移動・dismiss は保留移動と確定を取り消す。
  保留操作の実行中も、各操作で入力またはカーソルが変わった時点で現在の `input_generation` を無効化し、
  残りの保留操作を取り消す。
  入力変更後は先頭の窓・未選択で始める。入力変更・dismiss・確定・session 変更後の古い応答は
  一覧を復活させず、順序の逆転した応答で選択を巻き戻さない。
  有効な再描画要求の失敗時は一覧と保留操作を破棄し、入力は保つ。要求と確定を自動 replay しない。
...
- fork 内で候補を収集するときは、他プラグイン(zsh-autosuggestions など)が定義した
  ...
  親の対話シェルの状態には影響しない。
- 保証範囲は作者環境の zsh-abbr のみ。
- 外部シンタックスハイライトプラグインとの併用はサポートせず、挙動は仕様外とする。
  併用の検出・警告は行わない。
  一般のプラグイン互換性は保証しない。

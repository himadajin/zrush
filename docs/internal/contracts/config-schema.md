# config-schema: config.toml Specification

`~/.config/zrush/config.toml`(`$XDG_CONFIG_HOME` 優先。cli-protocol.md 参照)の
テーブル・キー・型・既定値・検証規則。この文書が真実であり、コードはこれに追従する。

設定の反映はプロンプト表示ごとの mtime 確認による自動反映(明示リロードなし)。
ファイル不在は正常で、全既定値で動作する。

## [display]

| キー | 型 | 既定値 | 制約 | 意味 |
|---|---|---|---|---|
| `max-lines` | 整数 | `10` | 1〜1000 | 候補一覧の最大行数(通常補完の位置表示行も含む。予算 1 行では位置表示を省略。表示時はプロンプトと入力行を 1 行確保するため、さらに `$LINES - 1` でクリップ) |
| `delay-ms` | 整数 | `30` | 0〜10000 | worker が捕獲を要し得る通知を採用するまでの静穏期間(ミリ秒)。0 は即時 |
| `min-input` | 整数 | `0` | 0〜100 | 一覧表示を開始する現在語の最小文字数(空の引数語は長さ 0。空バッファは別規則で常に対象外) |

- `min-input` と空バッファの抑止規則が効くのは入力に追従する自動表示のみで、
  明示操作である履歴メニュー(behavior.md)には適用しない。
  バッファ装飾にも `min-input` は効かない(1 文字目から着色する。`[syntax]`)。
- `delay-ms` の待ちは worker 側で起こる。
  zsh は抑止規則を通ったバッファ変化を直ちに入力通知として送り、その通知が運ぶ `delay-ms` の分だけ
  worker が新しい通知を待って、最新のものだけを採用する
  (cli-protocol.md「Input Notifications and Worker Events」、behavior.md「Candidate Collection」)。
  分解能は 1 ミリ秒で、指定した値がそのまま静穏期間になる。
  静穏期間中に届いた新しい通知は期間を張り直すため、打鍵が続く間は通知が採用されない。
  `delay-ms = 0` は通知を受理と同時に採用することを意味し、打鍵ごとに settle する。
  静穏期間が掛かるのは捕獲を要し得る通知だけであり、
  candidate store だけで解決する通知(空語収集キャッシュのヒット)は
  `delay-ms` の値によらず受理と同時に `plan-ready` が返る。
- `delay-ms` はバッファ装飾には効かない。
  worker は通知の受理時に字句解析して直ちに結果を返す(`[syntax]`、behavior.md「Buffer Syntax Highlighting」)。

## [display.highlight]

一覧の装飾。値は zsh の highlight 指定文字列(`region_highlight` の spec:
`standout`, `bold`, `underline`, `fg=blue,bold` など)をそのまま zsh へ渡す。

| キー | 型 | 既定値 | 意味 |
|---|---|---|---|
| `selected` | 文字列 | `"standout"` | 選択中セルの装飾 |
| `match` | 文字列 | `"underline"` | マッチ箇所の装飾(match-text 表示時のみ。選択中セルには適用しない) |
| `heading` | 文字列 | `"bold"` | グループ見出し行の装飾 |
| `history-number` | 文字列 | `"faint"` | 履歴イベント番号の装飾(選択中セルには適用しない) |

- 検証は「文字列であること」まで。spec の解釈は zle に委ねる(不正な spec は zle が無視する)。
- **空文字列は「装飾なし」**(region_highlight エントリを追加しない)。

## [matching]

| キー | 型 | 既定値 | 制約 | 意味 |
|---|---|---|---|---|
| `mode` | 文字列 | `"typo"` | `"prefix"` / `"substring"` / `"typo"` | マッチ強度。後のものほど広く拾う(意味論は cli-protocol.md) |
| `smart-case` | 真偽値 | `true` | — | クエリが全小文字なら大文字小文字を無視 |

## [insert]

| キー | 型 | 既定値 | 制約 | 意味 |
|---|---|---|---|---|
| `tab` | 文字列 | `"menu"` | `"common-prefix"` / `"menu"` / `"insert"` | Tab の挙動: 共通接頭辞を挿入(伸びなければ先頭候補を確定挿入)/ 選択メニューに入る / 先頭候補を即挿入 |
| `trailing-space` | 真偽値 | `true` | — | 補完候補の確定時に末尾スペースを付与(候補が `-S` 等の接尾辞を持つ場合、およびディレクトリ候補の合成 `/` が付く場合は付与しない) |

## [history]

| キー | 型 | 既定値 | 制約 | 意味 |
|---|---|---|---|---|
| `limit` | 整数 | `5000` | 1〜20000 | 履歴メニューを開くたびに worker が history index の新しい側から走査するエントリの最大数 |

- 走査範囲は `min(limit, index の件数)` 件である。
  重複除去はこの範囲の中で行い(同一の履歴行は最も新しい出現だけを残す)、
  範囲の外から補充しないため、一覧の対象になる行数は走査件数より少なくなり得る
  (cli-protocol.md「history profile」)。
  `limit` をセッション途中で上げても、index の件数がそれに満たない間は走査範囲は index 全体である。
- 上限 20000 は worker の history index の retention cap と同値である。
  index はこの件数を超えた分を古い側から捨てるが、走査範囲は cap 以下であるため、
  この破棄が一覧に影響することはない。
- `limit` が有界化するのは**走査する履歴エントリ数だけ**である
  (1 行の長さに上限を設けないため、`limit` だけでは走査するバイト量が決まらない)。
- index を作り直すときに zsh が合成する payload の総バイト数は、`limit` とは別に置いた固定上限で
  有界化する(behavior.md「History Menu」)。
  合成は `limit` で絞らないため、`limit` をいくつにしても同期経路が扱うバイト量はその固定上限を超えない。
- 履歴候補の表示行数は `[display].max-lines` を共用する(履歴専用のキーは設けない)。

## [syntax]

コマンドラインバッファ自身のシンタックスハイライト。

| キー | 型 | 既定値 | 制約 | 意味 |
|---|---|---|---|---|
| `enabled` | 真偽値 | `true` | — | バッファのシンタックスハイライトを行うか |

- `enabled = false` は**解析ごと止める**。zsh は入力通知の `buffer` field を空で送る。
  worker はこの設定を知らないが、空の入力には token が 1 個も無いため、
  字句解析も filesystem 参照も起こらない。zsh はバッファ装飾のエントリを持たない
  (cli-protocol.md「Input Notifications and Worker Events」)。
- これは「`[syntax.highlight]` の全キーが空文字列」とは別概念である。
  後者では解析は通常どおり走り、装飾だけが付かない。
- `enabled = true` の間、`[display].min-input` による抑止は入力通知を止めない。
  1 文字目から着色するためであり、一覧側の帰結は zsh が捨てる(behavior.md「Candidate Collection」)。
  空バッファの抑止は `enabled` によらず通知を止める(装飾ゼロという結果を zsh だけで決められるため)。

## [syntax.highlight]

バッファの装飾。値は `[display.highlight]` と同じく zsh の highlight 指定文字列
(`region_highlight` の spec)をそのまま zsh へ渡す。
キーは字句解析器の分類(`../specs/syntax.md`「Token kinds」)の kebab-case であり、
`syntax-highlight` event の `role` と 1 対 1 に対応する(cli-protocol.md)。

| キー | 型 | 既定値 | 意味 |
|---|---|---|---|
| `command` | 文字列 | `"fg=green"` | `$PATH` 上またはパス指定で実在する外部コマンド |
| `reserved` | 文字列 | `"fg=yellow"` | 予約語(`if`, `for`, `[[` など) |
| `alias` | 文字列 | `"fg=green"` | エイリアス名 |
| `function` | 文字列 | `"fg=green"` | 関数名 |
| `builtin` | 文字列 | `"fg=green"` | ビルトイン名 |
| `precommand` | 文字列 | `"fg=green,underline"` | `command` / `exec` / `nohup` / `sudo` |
| `unknown` | 文字列 | `"fg=red,bold"` | 解決できなかったコマンド語 |
| `assignment` | 文字列 | `""` | 前置代入(`NAME=value`) |
| `option` | 文字列 | `""` | オプション語(`-a`, `--all`) |
| `redirect` | 文字列 | `""` | リダイレクト演算子 |
| `operator` | 文字列 | `""` | 制御演算子・区切り |
| `comment` | 文字列 | `"fg=black,bold"` | `interactive_comments` 有効時のコメント |
| `single-quote` | 文字列 | `"fg=yellow"` | `'...'` の範囲 |
| `double-quote` | 文字列 | `"fg=yellow"` | `"..."` の範囲 |
| `dollar-quote` | 文字列 | `"fg=cyan"` | `$'...'` の範囲 |
| `escape` | 文字列 | `"fg=cyan"` | `\` とその直後の 1 バイト |
| `substitution` | 文字列 | `""` | `$(...)` / `${...}` の範囲 |
| `path` | 文字列 | `"underline"` | 実在する literal path の語 |

- 検証は `[display.highlight]` と同じく「文字列であること」まで。spec の解釈は zle に委ねる。
- **空文字列は「装飾なし」**(region_highlight エントリを追加しない)。
- 字句解析器が `Word` に分類する語(通常の引数)はキーを持たず、常に無装飾である。
  キーの粒度は字句解析器の分類が区別するものに限り、
  それを超える細分化(文字列を single / double で分ける以上の分割など)はしない。
- 意味分類のキー(`command`〜`option`)と装飾範囲のキー(`single-quote`〜`path`)は
  同じ範囲に重なり得る。重なりは event の並び順で解決し、**後のエントリが勝つ**
  (cli-protocol.md「`syntax-highlight` body (Buffer Highlight Stream)」)。
  実際の並びは「意味分類 → 装飾範囲 → `path`」であるため、
  たとえば実在するクォート済みパスは `path` の装飾になる。

## [keybind]

| キー | 型 | 既定値 | 意味 |
|---|---|---|---|
| `select-next` | 文字列 / 文字列の配列 | `["down", "ctrl-n"]` | 選択開始 / 次候補(表示順 = 列優先グリッドの列内下方向)。履歴一覧では 1 つ新しい履歴へ(位置 1 では履歴メニューを閉じる) |
| `select-prev` | 文字列 / 文字列の配列 | `["up", "ctrl-p"]` | 前候補(補完一覧の先頭候補では選択解除)。非選択時は履歴メニューを開き、履歴一覧では 1 つ古い履歴へ |
| `select-left` | 文字列 / 文字列の配列 | `["left", "ctrl-b"]` | 選択中のみ: 左の列へ(グリッド行数ぶん戻る。グループ範囲でクランプ)。単一列の履歴一覧では位置 1(最新)へ |
| `select-right` | 文字列 / 文字列の配列 | `["right", "ctrl-f"]` | 選択中のみ: 右の列へ(グリッド行数ぶん進む。グループ範囲でクランプ)。単一列の履歴一覧では表示中の最終位置(最古)へ |
| `confirm` | 文字列 / 文字列の配列 | `["enter"]` | 確定(挿入のみ。実行しない) |
| `dismiss` | 文字列 / 文字列の配列 | `["ctrl-g"]` | 却下(一覧を閉じる) |

- 割り当て可能なアクションはこの 6 つのみ。
  Tab は `[insert].tab` の挙動に従う(`menu` のときは選択開始キーを兼ねる)ため独立アクションにしない。
- **1 アクションに複数キーを割り当てられる**。単一の文字列は 1 要素配列と等価。
  - 配列指定は既定リスト全体の**置き換え**(既定へのマージはしない)。
  - 空配列 `[]` は「キーを割り当てない」(そのアクションの無効化。有効な指定)。
  - 無効な記法・Tab キー(`^I`)の要素は**要素単位**で捨てて警告する。
    非空の指定から有効な要素が 1 つも残らなかった場合は既定リストへフォールバックして警告。
  - 配列内の重複(正規化後)は除去して警告。
  - 配列の要素が文字列でない場合、その要素を捨てて警告(他の要素は有効)。
    全要素が捨てられた非空の配列は、上記の既定リストフォールバックに従う。
- `select-left` / `select-right` を含む全アクションは原則として**選択中のみ**キーを奪う。
  例外は 3 つ: `dismiss` は非選択でも一覧表示中なら一覧を閉じ、
  `select-next` は一覧表示中なら非選択でも選択を開始し(behavior.md の優先順位規則に従う)、
  `select-prev` は非選択時に履歴メニューを開く(behavior.md「History Menu」節)。
  それ以外の非選択時は前任者チェーンへフォールバックするため、既定の ← → / ctrl-b / ctrl-f は
  カーソル移動、ctrl-n は履歴移動のまま(ctrl-n は ↓ と同じ優先順位規則に従う)。
- 既定では ↑ と ctrl-p の両方が `select-prev` であり、どちらも非選択時に履歴メニューを開く。
  ctrl-p を素の履歴移動のまま使いたい場合は `select-prev = ["up"]` のように配列から外す
  (配列指定は既定リストの置き換えであるため、この 1 行で ctrl-p は前任者チェーンへ戻る)。
- **Tab キー(`tab` / `ctrl-i`、正規化後 `seq:^I`)をアクションに割り当てることはできない**
  (該当要素を捨てて警告する。
  zsh 側は ^I を常に固定の Tab 挙動フックに束縛するため、黙って上書きされる事故を防ぐ)。
- `dismiss` の既定は `ctrl-g`(Escape 単独は `\e` プレフィックスと曖昧で
  KEYTIMEOUT 待ちが発生するため既定にしない。ユーザーが明示設定することは可能)。

### Key Notation

小文字で書く。修飾は `ctrl-` / `alt-` を前置(`alt` は ESC プレフィックス送出を仮定)。

| 記法 | 正規化結果(cli-protocol.md の形式) |
|---|---|
| `a`〜`z`, `0`〜`9`, 記号 1 文字 | `seq:<その文字>`(ただし `^` は `seq:\^`、`\` は `seq:\\` — bindkey のキャレット/バックスラッシュ記法として誤解釈されないようエスケープして出力する) |
| `ctrl-a`〜`ctrl-z` | `seq:^A`〜`seq:^Z` |
| `ctrl-space` | `seq:^@` |
| `alt-<文字>` | `seq:^[<文字>`(`<文字>` が `^` / `\` の場合は単字形式と同じエスケープを適用: `alt-^` → `seq:^[\^`) |
| `enter` | `seq:^M` |
| `tab` | `seq:^I` |
| `escape` | `seq:^[`(前述の曖昧さ注意) |
| `space` | `seq:" "` 相当(空白 1 文字) |
| `up` / `down` / `left` / `right` | `key:up` など(zsh が `$terminfo[kcuu1]` 等+CSI/SS3 両系統で解決) |
| `shift-tab` | `key:shift-tab`(`$terminfo[kcbt]`) |
| `home` / `end` / `pgup` / `pgdn` / `delete` | `key:<名前>`(terminfo 解決) |

- 上記にない記法は検証エラー(該当要素を捨てて警告。[keybind] の要素単位規則を参照)。
- 同一キーを複数アクションに割り当てた場合は検証エラー(後勝ちにせず、関与した全アクションを
  既定リストへ戻して警告。意図しない上書きの黙認を避ける)。**比較は正規化後の値で行う**
  (`enter` = `ctrl-m` = `^M`、`tab` = `ctrl-i` = `^I` を同一キーとして検出する)。
  既定リスト同士は互いに素なので、この解決は必ず収束する。
- `key:` 形式の terminfo 解決は zsh 側で行うため、Rust の検証は記法の妥当性まで。
  端末の terminfo に該当エントリが無い場合(例: `$terminfo[kcbt]` が空)、
  zsh 側はそのキーの bindkey をスキップして警告する(他のキーは通常どおり適用)。

## Validation and Error Handling

- config ファイルが存在しない場合は正常として警告せず、全既定値で動作する。
  それ以外の読み取り失敗(権限エラーなど)は警告を出し、全既定値にフォールバックする。
- **起動は決して失敗させない**。検証は項目単位で行い、不正な項目のみ既定値にフォールバックする:
  - 型不一致(文字列に整数など)/ 制約違反(範囲外・未知の列挙値)/ 未知のキー・未知のテーブル(typo 検出のため警告対象)
  - TOML 構文エラーの場合のみファイル全体を不正として全既定値で動作
- 各エラーは `ZRUSH_CFG_WARNINGS` に 1 件 1 メッセージで出力する(cli-protocol.md)。
  メッセージは「どのキーが・なぜ・どの値にフォールバックしたか」を 1 行で含む。
  例: `config: [display] max-lines: expected integer 1..1000, got "abc"; using default 10`
- **表示先とタイミング(zsh 側の責務)**: config を(再)読み込みしたプロンプト表示時に、
  警告配列を 1 行ずつ stderr へ表示する。再読み込みは mtime 変化時のみ行われるため、
  config が変化しない限り同一警告が再表示されることはない(毎プロンプトのスパム防止)。

...
- `--producer`: stdin に流す payload の由来(producer profile 名)。省略できない。
  レコード解釈・レイアウト・ハイライト・挿入テキスト構築は producer に依らず同一で、
  この引数が変えるのは結果順の規範だけである(「マッチング・ランキングの意味論」節)。
...
- 候補レコードは新しい順(最新の履歴行が先頭)。
  ...
  位置番号と画面上の配置の対応は「表示行の中身」節の列優先規則による)。
...
- ランキング: 結果順は `--producer` で決まる。
  ...
  - `--producer history`: stdin での出現順をそのまま保つ(マッチ品質で並べ替えない)。
    ティアはこのとき「候補を一覧に含めるかの判定」と「ハイライト範囲の計算」にのみ使う。
